...
async function route(
  ...
  // Guarded as well as hidden. The UI stops offering this when the flag is off,
  // but a bookmark or a stale tab should not walk into a path that cannot
  // complete.
  if (path === "/connect/dmdrop/start" && post) {
    return brokerEnabled(env)
      ? startBrokerConnect(env, request)
      : redirect("/settings");
  }
