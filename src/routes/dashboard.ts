...
function healthBanner(account: AccountRow) {
  ...
  // Meta's own words go in too, never instead. Our explanation is a guess from
  // a health code; the note is what the platform actually said. When the app
  // itself is blocked at Meta's end, "reconnect" is wrong advice that a person
  // can follow all day without getting anywhere — which is precisely what it
  // cost us to learn.
  return html`<div class="notice bad">
    <strong>@${account.username} is paused.</strong>
    ${explanation[account.health] ?? "Reconnect this account."}
    ${account.health_note ? html`<span class="small muted">Instagram said: ${account.health_note}</span>` : raw("")}
