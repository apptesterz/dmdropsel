...
/**
 * Whether to offer the one-tap path at all.
 *
 * Off by default. Our relay app holds Standard access, which Meta limits to
 * accounts holding a role on the app — so a customer tapping it authorises on
 * Instagram and then has every call for their account refused. Offering a
 * button that cannot work is worse than not offering it.
 *
 * Everything behind the flag stays wired and tested: flip BROKER_ENABLED to "1"
 * on the day Advanced access is granted.
 */
export function brokerEnabled(env: Env): boolean {
  return env.BROKER_ENABLED === "1";
}
