...
import { connectMode, brokerEnabled } from "./broker";
...
export async function renderSettings(
  ...
  const body = html`
    ...
    ${
      /*
       * Offered only while the one-tap path can actually complete. The escape
       * hatch below — switching an existing dmdrop-mode instance back to its
       * own Meta app — is never hidden, because someone already on that path
       * must always be able to leave it.
       */
      mode === "own" && brokerEnabled(env)
        ? html`<div class="card">
