...
import { brokerEnabled } from "./broker";
...
export async function renderSetup(
  ...
  const body =
    step === 1
      ? step1()
      : step === 2
        ? step2(origin, session!, await ensureVerifyToken(env), Boolean(await licenceKey(env)), brokerEnabled(env))
        : step === 3
          ? step3(session!)
          : step4();
  ...
  return html`
    <div class="lede-l">
      ...
      <p>
        ${
          brokerOn
            ? "Two ways. Pick either — you can change your mind later without losing anything."
            : "This is the one part that needs a computer, and about fifteen minutes of it. Everything afterwards happens on your phone."
        }
      </p>
    </div>
    ...
    ${
      /*
       * The licence used to live inside the one-tap card, which meant hiding
       * that card hid the only place to enter a key. It is the product key
       * either way, so it stands on its own.
       */
      hasLicence
        ? raw("")
        : html`<div class="card">
            <span class="overline">Licence</span>
            <form method="post" action="/setup/licence" class="mt-12">
    ...
    ${
      /*
       * One-tap connect, hidden unless BROKER_ENABLED is set.
       *
       * Our relay app holds Standard access, which Meta limits to accounts with
       * a role on the app. A customer tapping this would authorise on Instagram
       * and then have every API call for their account refused — a dead end
       * that looks like a bug. Advanced access needs business verification and
       * Tech Provider review; the flag flips back on the day that lands, and
       * every route behind it is still here and still tested.
       */
      brokerOn && hasLicence
        ? html`<div class="card">
            ...
            <div class="tile">
              ...
              <div class="grow">
                ...
                <div class="sub">
                  ...
                  comments and DMs reach you through our relay, and everything
                  else — your automations, your contacts, your link clicks, your
                  Instagram token — stays on this instance.
                </div>
              </div>
            </div>
            <form method="post" action="/connect/dmdrop/start" class="mt-12">
              ${csrfField(session.csrf)}
    ...
    <div class="card">
      <span class="overline">${brokerOn ? "The independent way" : "What you are about to do"}</span>
      <p class="small muted mt-0">
        ${
          brokerOn
            ? raw("About 15 minutes in Meta's console, and afterwards nothing of ours is in the path at all — not even for incoming messages. Choose this if you would rather depend on nobody, or if you are running this for clients. The steps are below.")
            : raw("You create your own app on Meta's developer site, then paste four values between it and this page. Afterwards nothing of ours sits in the path at all — not even for incoming messages — which is what makes this yours rather than rented.")
        }
      </p>
    </div>
